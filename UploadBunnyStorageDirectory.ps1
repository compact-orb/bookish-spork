...
.DESCRIPTION
    ...
    - BUNNY_STORAGE_ENDPOINT: The endpoint for the storage zone.
...
Get-ChildItem -Path $Path -Recurse -Name -File | ForEach-Object -Parallel {
    ...
    for ($attempt = 1; $attempt -le $maxRetries; $attempt++) {
        try {
            ...
            Invoke-RestMethod -Uri "https://$env:BUNNY_STORAGE_ENDPOINT/$env:BUNNY_STORAGE_ZONE_NAME$using:Destination/$filePath" -Headers @{"accept" = "application/json"; "accesskey" = $env:BUNNY_STORAGE_ACCESS_KEY } -Method PUT -ContentType "application/octet-stream" -InFile "$using:Path/$filePath" | Out-Null
